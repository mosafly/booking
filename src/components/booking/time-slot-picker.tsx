...
import { getEquipmentType, generateTimeSlots, formatDuration, EquipmentType, TimeSlot } from "@/lib/utils/reservation-rules";
...
const TimeSlotPicker: React.FC<TimeSlotPickerProps> = ({
  ...
  const slotsByStartTime = validSlots.reduce((acc: Record<number, TimeSlot[]>, slot: TimeSlot) => {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {slotsForTime.map((slot: TimeSlot, index: number) => {
                  ...
                  return (
                    <button
                      key={index}
                      onClick={() => handleSlotSelection(slot.startTime, slot.endTime)}
                      className={`
