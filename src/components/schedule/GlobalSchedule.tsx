...
courts:court_id(name),
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'court': return 'bg-blue-100 border-blue-300 text-blue-800';
            ...
            {getTimeSlots().map(time => (
              <div key={time} className="grid grid-cols-8 gap-px bg-gray-200">
                ...
                {getWeekDays().map(day => {
                  ...
                  return (
                    <div key={`${day}-${time}`} className="bg-white p-1 min-h-20">
                      {items.map(item => (
                        <div
                          key={item.id}
                        ...
                        >
                          ...
                          {item.coach_name && (
                            <div className="text-xs opacity-75">{item.coach_name}</div>
                          )}
              ...
              <div key={day.toISOString()} className="border rounded-lg p-4">
                ...
                {dayItems.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {dayItems.map(item => (
                      <div key={item.id} className={`p-3 rounded-lg border ${getTypeColor(item.type)} ${getStatusColor(item.status)}`}>
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            {item.court_name && (
                              <p className="text-sm text-gray-600">{item.court_name}</p>
                            )}
                            {item.coach_name && (
                              <p className="text-sm text-gray-600">Coach: {item.coach_name}</p>
                            )}
                          </div>
                          <div className="text-right">
                            {item.type === 'gym' && item.price && (
                              <p className="font-semibold">{item.price / 100} FCFA</p>
                            )}
                            {item.type === 'gym' && item.max_participants && (
                              <p className="text-sm">Max: {item.max_participants}</p>
                            )}
                          </div>
                        </div>
                      </div>
