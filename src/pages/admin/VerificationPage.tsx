...
export function VerificationPage() {
    ...
    const callVerificationApi = useCallback(async (action: string, body: object) => {
    ...
    const fetchReservationDetails = useCallback(async (id: string) => {
        ...
        try {
            const data = await callVerificationApi('get_reservation', { verificationId: id });
    ...
    const markAsUsed = useCallback(async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : "Failed to mark booking as used.");
        } finally {
            setIsLoading(false);
        }
    ...
    const getStatus = () => {
        if (error) return {
            ...
            badgeText: "Invalid", statusText: "Verification Failed"
        };
        if (reservationData?.verification_used_at) return {
            ...
            badgeText: "Already Used", statusText: "Already Checked In"
        };
        if (reservationData) return {
            ...
            badgeText: "Valid", statusText: "Ready for Check-in"
        };
        return null;
    };
    ...
    return (
        <div className="min-h-screen bg-background py-8 px-4">
            <div className="max-w-md mx-auto space-y-6">
                {isLoading && !reservationData && <Card><CardContent className="pt-6 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto mb-2" /><p>Loading booking details...</p></CardContent></Card>}
                    ...
                    <Card className={`border-2 ${status.borderColor} ${status.bgColor}`}>
                        <CardContent className="pt-6 text-center">
                            <div className="flex flex-col items-center space-y-4">
                                {status.icon}
                                <h2 className={`text-xl font-bold ${status.textColor}`}>{status.statusText}</h2>
                                {reservationData && (
                                    <div>
                                        <p className="text-2xl font-bold">{reservationData.user_name}</p>
                ...
                {reservationData && (
                    <Card>
                        ...
                        <CardContent className="space-y-2">
                            <p><strong>Court:</strong> {reservationData.court_name}</p>
                            <p><strong>Time:</strong> {new Date(reservationData.start_time).toLocaleString()}</p>
                            <p><strong>Status:</strong> {reservationData.verification_used_at ? `Used at ${new Date(reservationData.verification_used_at).toLocaleString()}` : 'Not used'}</p>
                            <p><strong>Email:</strong> {reservationData.user_email}</p>
                            {!reservationData.verification_used_at && (
                                <Button onClick={markAsUsed} disabled={isLoading} className="w-full mt-4">
                                    {isLoading ? <Loader2 className="animate-spin" /> : "Mark as Used"}
                                </Button>
                            )}
                        </CardContent>
                    </Card>
